...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	float joint = ref[action/2] + ((action % 2 == 0) ? actionJointDelta : -1 * actionJointDelta); 
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
            	...
            	if (distDelta<0.002 && distDelta>-0.002)
				{
				...
				else
				{
					...
					//rewardHistory = avgGoalDelta * REWARD_WIN * 100;
